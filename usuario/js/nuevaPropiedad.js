...
document.querySelector("#guardar").onclick = () => {
  ...
  if (id > -1) {
    tipoPeticion = "post";
    urlPeticion = `https://inventapro.site/api/src/public/updPropiedad?${datos}&id=${id}`
  } else {
    tipoPeticion = "post";
    urlPeticion = `https://inventapro.site/api/src/public/insPropiedad?${datos}`
  }
  $.ajax({
    type: tipoPeticion,
    url: urlPeticion,
...
function getPropiedad(id) {
  $.ajax({
    type: "get",
    url: `https://inventapro.site/api/src/public/listPropiedad/${id}`,
    dataType: "JSON",
    beforeSend: function () {
      //document.querySelector(".cargando").style.visibility="visible";
    },
