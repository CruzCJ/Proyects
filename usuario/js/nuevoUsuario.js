...
document.querySelector("#guardar").onclick = () => {
  ...
  if (id > -1) {
    tipoPeticion = "post";
    urlPeticion = `https://inventapro.site/api/src/public/updUsuario?${datos}&id=${id}`
  } else {
    tipoPeticion = "post";
    urlPeticion = `https://inventapro.site/api/src/public/insUsuario?${datos}`
  }
  $.ajax({
    type: tipoPeticion,
    url: urlPeticion,
...
function getUsuario(id) {
  $.ajax({
    type: "get",
    url: `https://inventapro.site/api/src/public/listUsuarios/${id}`,
    dataType: "JSON",
    beforeSend: function () {
      //document.querySelector(".cargando").style.visibility="visible";
    },
